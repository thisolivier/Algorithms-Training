...
func climbingLeaderboard(scores: [Int], alice: [Int]) -> [Int] {
        ...
        while aliceScore.rank == -1, !emergencyStop {
            ...
            if score >= currentLeaderboardItem.0 && index == 0 {
                ...
                if (score < currentLeaderboardItem.0 && score >= belowLeaderboardItem.0) {
                    aliceScore.rank = currentLeaderboardItem.1
                    aliceScore.log("🏆 - Am below the next, but above or equal to index \(index)")
                }
            } else {
    ...
    for intTuple in sneakyTree {
        var flag = ""
        if let aliceScore = aliceScores.filter( { (aliceScore: AliceScore) in
            return aliceScore.score == intTuple.0
        }).first {
            flag = " - [\(aliceScore.score) on \(aliceScore.index)th try]"
        }
...
climbingLeaderboard(scores: context, alice: alice)
